...
X_sym, y_sym, h0_sym = add_datasets_to_graph([X, y, h0], ["X", "y", "h0"], graph)
...
def step(x_t, h_tm1):
    h_t = tanh_recurrent_layer([x_t], [h_tm1], graph, 'l1_rec', random_state)
    return h_t

# the hidden state `h` for the entire sequence
h, _ = rnn_scan_wrap(step, name='main_scan', sequences=[proj_X],
                     outputs_info=[h0_sym])
...
fit_function = theano.function([X_sym, y_sym, h0_sym], [cost],
                               updates=updates)

...
X_clean = [l[:-1] for l in symbols]
y_clean = [l[:-1] for l in symbols]
X_clean = X_clean[:-(len(X_clean) % minibatch_size)]
y_clean = y_clean[:-(len(y_clean) % minibatch_size)]
...
# Recurrent
h = easy_lstm_recurrent([proj_X], X_mask_sym,
...
epoch_results = iterate_function(
    ...
    list_of_minibatch_functions=[text_minibatcher], n_epochs=1000,
